...
pub(crate) fn create_drop_pipe() -> Result<(PipeRead, PipeWrite), Error> {
    let (read, write) = pipe2(OFlag::O_CLOEXEC)
        .map_err(Error::PipeCreate)?;
    // Safety: Creating OwnedFds from owned fds (that were just created)
    unsafe {
        Ok((PipeRead(OwnedFd::from_raw_fd(read)), PipeWrite(OwnedFd::from_raw_fd(write))))
    }
}
...
pub fn run(context: &Arc<Context>, setmap: &SetMap, max_length: usize, receiver: &Receiver<Atom>, pipe_read: PipeRead) {
    ...
    let mut poll_fds = [PollFd::new(&con_fd, PollFlags::POLLIN), PollFd::new(&pipe_read.0, PollFlags::all())];
